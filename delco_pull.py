...
count = 0
err_count = 0
async def get_data(session, row, csv_columns, csv_file_path):
    global count
    global err_count
    count += 1
    try:
...
async def main():
    ...
    # csv_df = pd.read_csv("swarthmore_parcels.csv")
    
    # Define the expected CSV columns
    csv_columns = [
        "Parcel_Site_Location", "Parcel_Legal_Description", "Parcel_Map_Number", "Parcel_Municipality",
        "Parcel_School_District", "Parcel_Property_Type", "Parcel_Homestead_Status_-_Next_School_Bill_Cycle",
        "Parcel_Homestead_Status_-_Current_School_Bill_Cycle", "Parcel_Homestead_%", "Parcel_Homestead_Approved_Year",
        "Parcel_Additional_Info", "Parcel_Veteran's_Exemption", "Owner_Name(s)", "Owner_Care_Of", "Owner_Mailing_Address",
        "Assessment_Value", "Assessment_Date", "Assessment_Reason_for_Change", "Assessment_Comment", "Residential_Class",
        "Residential_Grade", "Residential_CDU", "Residential_Style", "Residential_Acres", "Residential_Year_Built_/_Effective_Year",
        "Residential_Remodeled_Year", "Residential_Base_Area", "Residential_Finished_Bsmt_Area", "Residential_Number_of_Stories",
        "Residential_Exterior_Wall", "Residential_Basement", "Residential_Physical_Condition", "Residential_Heating",
        "Residential_Heating_Fuel_Type", "Residential_Heating_System", "Residential_Attic_Code", "Residential_Fireplaces_1_Story/2_Story",
        "Residential_Parking", "Residential_Total_Rooms", "Residential_Full_Baths", "Residential_Half_Baths",
        "Residential_Total_Fixtures", "Residential_Additional_Fixtures", "Residential_Bed_Rooms", "Residential_Family_Room",
        "Residential_Living_Units", "Parcel_ID", "URL", "Mortgage_Company", "Mortgage.Service_Co_Name", "geometry"
    ]

    # Try to read the existing CSV file or create an empty DataFrame with the defined columns if the file doesn't exist
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        for index, row in query_df.iterrows():
            task = asyncio.ensure_future(get_data(session, row, csv_columns, csv_file_path))
            tasks.append(task)
